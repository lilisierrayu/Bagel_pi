...
#SBATCH --qos=normal
#SBATCH --job-name=folding_conditioning
...
srun torchrun --nnodes=$SLURM_NNODES --nproc_per_node=8 \
  ...
  --dataset_config_file data/configs/seedp1_0.2_diverse_batch_conditioning_allview_100steps.yaml  \
  --exp_name pi_diverse_batch_conditioning_100steps_seedp1_gpu${total_gpus}_seq${seq_len} \
  --wandb_runid 0 \
