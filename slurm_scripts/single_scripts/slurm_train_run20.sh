...
#SBATCH --qos=normal
#SBATCH --job-name=static_allview
...
srun torchrun --nnodes=$SLURM_NNODES --nproc_per_node=8 \
  ...
  --dataset_config_file data/configs/seedp1_0.2_arxs_ur5_allview_endspan.yaml \
  --exp_name pi_arxs_ur5_allview_seq_seedp1_gpu${total_gpus}_seq${seq_len} \
  --wandb_runid 0 \
