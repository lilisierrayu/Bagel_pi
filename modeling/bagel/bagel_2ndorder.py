...
class Bagel(PreTrainedModel):
    ...
    def generate_image(
        ...
        #     v1 = guided_velocity(model, x, t, cond, uncond, guidance_scale)
        #     x_euler = x + v1 * dt
        #     v2 = guided_velocity(model, x_euler, t_next, cond, uncond, guidance_scale)
        #     x = x + 0.5 * (v1 + v2) * dt
        total_l = len(timesteps) - int(using_2nd_order)
        for i, t in tqdm(enumerate(timesteps), total=total_l):
            if i == total_l - 1:
                print(i)
                break
            ...
            if using_2nd_order:
                ...
                timestep_next = torch.tensor([t_next] * x_t.shape[0], device=x_t.device)
                v_t_next = self._forward_flow(
                    x_t=x_t,
                ...
                x_t = x_t - 0.5 * (v_t + v_t_next).to(x_t.device) * dts[i]
